...
The **View** menu has additional panes that can be opened alongside the SQL editor - see [Views](/docs/user-guide/query#views) for the full list.
...
The timeline lanes are a visualization of exactly this list. The Events pane is where to go when you want the precise detail behind a tick on the timeline - which page a read touched, what lock mode was taken, or what the engine was waiting on. See [Events](/docs/user-guide/query/Events) in the user guide for everything it can do.
