...
using InternalsViewer.Connection.BackupFile.Mtf.Blocks.Os.Windows;
using InternalsViewer.Connection.BackupFile.Mtf.Streams;

namespace InternalsViewer.Connection.BackupFile.Mtf;

/// <summary>
/// BinaryReader extended with the MTF primitives used to parse MTF backup file format
/// </summary>
/// <remarks>
...
/// </remarks>
internal sealed class MtfReader : BinaryReader
{
    private delegate DescriptorBlock DescriptorBlockFactory(MtfReader reader);
    ...
    public MtfReader(string filename) : base(new FileStream(filename, FileMode.Open, FileAccess.Read))
    {
    }

    public MtfReader(byte[] data): base(new MemoryStream(data))
    {
    }

    public MtfReader(Stream stream) : base(stream)
    {
    }
