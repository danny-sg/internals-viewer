...
public static class NestedLoopsDescriber
{
    public static OperatorDescription Describe(NestedLoopsDefinition definition)
    {
        ...
        phases.Add(new AccessStrategyPhase
        {
            ...
            Title = "Verdict",
            Lead = "Weigh what the rebind returned against the join type. An inner join drops an outer row that matched nothing, an outer " +
                   "join emits it once with NULLs, and a semi join emits it at the first match and closes the inner early"
        });
        ...
        {
            Summary = "Join that reads its outer input once and re-runs the inner input from the start for every outer row, with the " +
                      "outer row's values bound into the inner side",
            IsStreaming = true,
