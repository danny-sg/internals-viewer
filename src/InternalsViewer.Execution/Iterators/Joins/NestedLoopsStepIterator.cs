                                            ...
                                            HeapFetchStepIterator heapIterator) : JoinStepIterator
{
    ...
    public override async Task OpenAsync(IteratorContext context, IteratorDefinition definition, CancellationToken cancellationToken)
    {
        ...
        await OuterInput.OpenAsync(context, cancellationToken);
    }
    ...
    internal override AccessStep Observe(AccessStep step, int side)
        => Attribute(step, side, OuterCounters.Add(CompletedInnerCounters));

    /// <summary>
    /// Builds the input the inner loop restarts, chosen by what the outer row supplies to each rebind
    /// </summary>
    private RebindableJoinInput CreateInner(IteratorDefinition definition)
        => definition switch
        {
            SeekDefinition seek
    ...
    private async Task<AccessStep> RebindAsync(IRecord record, CancellationToken cancellationToken)
    {
        ...
        var rebind = await InnerInput.RebindAsync(Context, record, RebindCount, cancellationToken);
