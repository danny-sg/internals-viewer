...
/// </remarks>
public sealed class JoinRecord : IRecord
{
    private JoinRecord(IRecord outer, IRecord inner, List<RecordField> fields)
    {
        Outer = outer;
        Inner = inner;
        Fields = fields;
    ...
    public static IRecord? Combine(IRecord? outer, IRecord? inner)
    {
        ...
        return new JoinRecord(outer, inner, [.. outer.Fields, .. inner.Fields]);
    }
}
