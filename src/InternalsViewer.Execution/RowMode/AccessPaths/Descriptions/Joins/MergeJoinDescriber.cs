...
namespace InternalsViewer.Execution.RowMode.AccessPaths.Descriptions.Joins;
    ...
    public static OperatorDescription Describe(MergeJoinDefinition definition)
    {
        ...
        if (definition.JoinType is not JoinType.Inner)
        {
            phases.Add(new AccessStrategyPhase
            {
                ...
                Title = "Preserve",
                Lead = "A preserved side emits its unmatched rows with NULLs as the walk passes them, and whatever is left of it is " +
                       "drained once the other input runs out."
            });
        }
        ...
        {
            Summary = "Join that reads both inputs once in the same key order, walking them together and advancing whichever side is " +
                      "behind.",
            IsStreaming = true,
