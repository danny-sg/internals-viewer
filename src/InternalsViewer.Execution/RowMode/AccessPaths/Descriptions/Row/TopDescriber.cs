...
namespace InternalsViewer.Execution.RowMode.AccessPaths.Descriptions.Row;
...
public static class TopDescriber
{
    public static OperatorDescription Describe(TopDefinition definition)
    {
        ...
        phases.Add(new AccessStrategyPhase
        {
            ...
            Title = "Row Count",
            Lead = $"The count of {definition.RowCount:N0} rows is fixed when the operator opens, and is pushed down as a row goal so " +
                   "the access path below can stop on its own."
        });
        ...
        phases.Add(new AccessStrategyPhase
        {
            ...
            Title = "Complete",
            Lead = "The operator ends at the row count, or earlier when the input runs out with fewer rows."
        });
