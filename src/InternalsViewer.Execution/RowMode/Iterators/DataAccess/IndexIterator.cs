...
using InternalsViewer.Execution.Common.DataAccess.Executors.Executors;
using InternalsViewer.Execution.Common.Interfaces.Pages;
using InternalsViewer.Execution.Common.DataAccess.Pages.Pages;
...
namespace InternalsViewer.Execution.RowMode.Iterators.DataAccess;
...
public sealed class IndexIterator(IPageService pageService, IRecordService recordService) : IteratorBase
{
    ...
    public override async ValueTask<IRecord?> GetRowAsync(CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (step is AccessStep.Stopped(InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.PageExhausted) && CurrentPage is { IsLeaf: true })
            {
    ...
    private bool HasNextRange(StopReason reason)
    {
        ...
        return reason == InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.RangeEnded
               || reason == InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.PageExhausted
               || (reason == InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.RowGoalMet && RowGoal != PlanRowGoal);
    }
