...
namespace InternalsViewer.Execution.RowMode.Iterators.Joins;
...
public sealed class MergeJoinIterator(IIteratorFactory factory) : JoinIterator
{
    ...
    protected override async IAsyncEnumerable<IRecord> RunAsync()
    {
        ...
        var reason = Outer.Iterator.CurrentRow is null
                     ? Outer.Iterator.StopReason ?? InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.PageExhausted
                     : Inner.Iterator.StopReason ?? InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.PageExhausted;
