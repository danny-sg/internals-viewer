...
public sealed class NestedLoopsIterator(IIteratorFactory factory) : JoinIterator
{
    ...
    protected override async IAsyncEnumerable<IRecord> RunAsync()
    {
        ...
        await EmitAsync(new AccessStep.Stopped(Outer.Iterator.StopReason ?? InternalsViewer.Execution.Common.AccessPaths.Results.StopReason.PageExhausted), CurrentToken);
    }
}
