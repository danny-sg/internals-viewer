...
///
/// A NULL key never equals anything, so a Build row with a NULL key is never added to the table and a Probe row with one never walks a
/// bucket.
///
/// Partitioning and spilling are not simulated, so the table is built entirely in memory at one recursion level.
/// </remarks>
public sealed class HashMatchStepService(IndexStepService buildService, IndexStepService probeService) : JoinStepService
{
    ...
    public async Task StartAsync(DatabaseSource database,
                                 HashSideDefinition buildInput,
                                 HashSideDefinition probeInput,
    ...
    {
        var build = new IndexRangeJoinInput(buildService, buildInput);

        var probe = new IndexRangeJoinInput(probeService, probeInput);
        ...
        BuildColumns = buildInput.JoinColumns;
        ProbeColumns = probeInput.JoinColumns;
        ...
        BuildRowEstimate = buildInput.RowEstimate;

        Residual = residual;

        CurrentEvaluationContext = evaluationContext;

        Table = new HashTable(bucketBits ?? JoinHash.BucketBitsFor(buildInput.RowEstimate));
        ...
        await build.StartAsync(database, cancellationToken, evaluationContext);

        await probe.StartAsync(database, cancellationToken, evaluationContext);
    ...
    private async IAsyncEnumerable<AccessStep> BuildAsync(InputCursor build)
    {
        ...
        await foreach (var step in build.AdvanceAsync().WithCancellation(CurrentToken))
        {
            yield return step;
        }
    ...
    private async IAsyncEnumerable<AccessStep> ProbeAsync(InputCursor probe)
    {
        ...
        await foreach (var step in probe.AdvanceAsync().WithCancellation(CurrentToken))
        {
            yield return step;
        }
    ...
    private AccessStep Stamp(AccessStep step, int source)
    {
        ...
        return step with { Source = source, Counters = BuildCounters.Add(ProbeCounters) };
    }
    ...
    /// <summary>
    /// Walks one input a row at a time, optionally holding what it reads in the input's buffer
    /// </summary>
    private sealed class InputCursor(JoinInput input, int source, HashMatchStepService owner, bool collectsRows = true)
    {
