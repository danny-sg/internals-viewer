...
internal class ObjectService
{
    ...
    public async Task<List<(int ObjectId, int IndexId, string Name, bool isUnique, bool isPrimaryKey, bool isUniqueConstraint)>> GetIndexesAndHeaps(string databaseName)
    {
        var connectionString = ConnectionStringHelper.GetConnectionString(databaseName);
        ...
        var sql = @"
            SELECT i.object_id, i.index_id, ISNULL(i.name, '(Heap)'), i.is_unique, i.is_primary_key, i.is_unique_constraint
            FROM   sys.indexes i
                   INNER JOIN sys.objects o ON i.object_id = o.object_id
            WHERE  o.is_ms_shipped = 0 AND i.type IN (0, 1, 2)";
