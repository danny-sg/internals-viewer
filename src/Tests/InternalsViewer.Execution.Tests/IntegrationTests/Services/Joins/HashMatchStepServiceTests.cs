...
public class HashMatchStepServiceTests(ITestOutputHelper testOutput)
{
    ...
    public async Task Overlapping_Ranges_Emit_Matching_Pairs()
    {
        ...
        Assert.Equal(HashMatchStepService.JoinSource, stopped.Source);
    ...
    public async Task Build_Completes_Before_The_First_Probe()
    {
        ...
        var buildRowsAfterProbe = steps.Skip(firstProbe)
                                       .Count(s => s is AccessStep.Row { EmittedRecord: not null }
                                                   && s.Source == HashMatchStepService.BuildSource);
    ...
    public async Task Resizing_Redistributes_Every_Row()
    {
        ...
        await context.Service.StartAsync(context.Database,
                                         SideInput(context.Unit, Between(100, 160)),
                                         SideInput(context.Unit, Between(105, 120)),
                                         CancellationToken.None);
    ...
    public async Task Left_Semi_Marks_Every_Matching_Entry_In_A_Chain()
    {
        ...
        var service = serviceHost.GetService<HashMatchStepService>();

        var side = new HashSideDefinition(unit.AllocationUnitId, unit.RootPage, [BigIntBetween(1, 20)], ["NumberField2"]);

        await service.StartAsync(database, side, side, CancellationToken.None, joinType: JoinType.LeftSemi);
    ...
    public async Task Steps_Are_Attributed_And_Counters_Are_Combined()
    {
        ...
        Assert.Contains(steps, s => s.Source == HashMatchStepService.BuildSource);
        Assert.Contains(steps, s => s.Source == HashMatchStepService.ProbeSource);
        Assert.Contains(steps, s => s.Source == HashMatchStepService.JoinSource);
    ...
    private sealed record NumberTableContext(DatabaseSource Database, HashMatchStepService Service, AllocationUnit Unit);
    ...
    private async Task<NumberTableContext> LoadNumberTableAsync()
    {
        ...
        return new NumberTableContext(database, serviceHost.GetService<HashMatchStepService>(), unit);
    }
    ...
    private static async Task<(List<AccessStep> Steps, List<(long Build, long Probe)> Pairs)> RunAsync(HashMatchStepService service)
    {
